...
{        
   ...
   public Client(String description, Position startposition, entityTypes type, byte charCode)
   {
       ...
       networkTimeoutSecs = ServerConfig.getNetworkTimeout();
   ...
   { 
       ...
       if (elapsed.getSeconds() >= networkTimeoutSecs)  // Send an update every n seconds
       {
    	   updateNow();
       }
   ...
   private void checkTimeout()
   {
       if (demoMode) return;   // Disable Timeout in demo mode
	    ...
	    if (elapsed.getSeconds() > networkTimeoutSecs)
	    {	        
	        if (!removeMeFlag)
	        {
